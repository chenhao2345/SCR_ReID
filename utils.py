...
def extract_feature( model, loader):
    ...
    # for (inputs, labels, _) in loader:  # MARS
    for (inputs, labels) in loader:  # Market
        # print(inputs.shape)
        # print(type(inputs))
        ff = torch.FloatTensor(inputs.size(0), 256*13).zero_()
        for i in range(2):
            if i == 1:
                # inputs = inputs.cpu()
...
class CenterLoss(nn.Module):
    ...
    def __init__(self, num_classes=1041, feat_dim=256, use_gpu=True):
